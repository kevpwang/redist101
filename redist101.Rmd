...
This vignette introduces redistricting analysis with algorithmic simulations--the state-of-art method to understand gerrymandering--using the `redist` package.
...
The first step in a `redist` analysis is to create the `redist_map` object, which stores the basic parameters of the redistricting problem. Here, we use `redist_map` objects that can be downloaded from the ALARM Project's [50-State Redistricting Simulations](https://alarm-redist.org/fifty-states/) using the `alarmdata` package.
